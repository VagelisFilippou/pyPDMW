    ...
    def __init__(self, derived_geometry, wing, parameters):
        ...
        for i in range(0, n_stringers_total):
            for j in range(0, derived_geometry.N_ribs):
                ...
                if stringers_nodes_x_incl_par[0, i, j] < spar_caps_xr_incl[0, 0, j] + parameters.stringers_tolerance:
                    stringers_nodes_x_incl_par[:, i, j] = np.nan
                    stringers_nodes_y_incl_par[:, i, j] = np.nan
                # if stringers_nodes_x_incl_par[0, i, j - 1] < spar_caps_xr_incl[0, 0, j - 1] + 0.06:
                #     stringers_nodes_x_incl_par[:, i, j - 1] = np.nan
                #     stringers_nodes_y_incl_par[:, i, j - 1] = np.nan
